#!/usr/bin/env python3
"""
Build a research area's literature_review.md from individual paper summaries.

Substantive source of truth: <area>/academic-papers/**/*.md
Bibliographic source of truth: <area>/literature-review/references.bib
"""
...
def discover_summaries(area: ResearchAreaConfig) -> list[Path]:
    paths: list[Path] = []
    academic_papers = area.academic_papers
...
def parse_summary(path: Path, area: ResearchAreaConfig) -> PaperSummary:
...
def load_bib_keys(references_bib: Path) -> list[tuple[str, str]]:
...
def build_markdown(papers: list[PaperSummary], area: ResearchAreaConfig) -> str:
    ...
    lines.append(
        ...
        f"`{area.summaries_path_display}`. Do not manually edit paper-level evidence here. "
        "Update the source summary and regenerate this file."
    )
    lines.append("")
    lines.append(
        "> **Sources of truth:** substantive content → paper summaries; "
        f"bibliographic metadata → `{area.references_bib_display}`."
    )
    lines.append("")
    ...
    lines.append(
        ...
        f"`{area.references_bib_display}`."
    )
    lines.append("")
    for key, label in load_bib_keys(area.references_bib):
...
def run_build(
    ...
    if not paths:
        print(
            f"WARNING: No paper summaries found under {area.academic_papers}/",
            file=sys.stderr,
        )
        return 1

    papers = [parse_summary(p, area) for p in paths]
    for p in papers:
        for w in p.warnings:
            print(f"WARNING [{p.path.name}]: missing or partial field — {w}", file=sys.stderr)

    md = build_markdown(papers, area)
    output_md = area.output_markdown
